...
rangeWeek <- function(data, fecha, semana){
  ...
  labelSem <- NULL
    ...
    for(i in 1:length(rFecha)){
      ...
      if(mi %in% mf){
        rangeSem <- paste0(paste(substring(rFecha[[i]][1], 9,10), substring(rFecha[[i]][2], 9,10), sep = "-"), monthE[mi])
      }else{
        rangeSem <- paste(paste0(substring(rFecha[[i]][1], 9,10), monthE[mi]), paste0(substring(rFecha[[i]][2], 9,10), monthE[mf]), sep = "-")
      }
