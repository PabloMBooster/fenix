biom_survey_map_modas_num = function (baseDat, outFolder = ".", outFile = "ModaPrincipal_numero.png", 
                                          ...
                                          cex_num = cex_num, legend2 = F, posX = NULL, posY = NULL, 
                                          ...
                                          LatTck = 2, n_perfil = 1) 
{
  ...
  if (Legend) {
    ...
    legend.krige(x.leg = c(posX, posX + diffX), y.leg = c(posY, 
                                                          posY + diffY), scale.vals = c(8, 12, 14.5), values = 0:20, 
                 vertical = T, col = scale.color2, offset.leg = 1)
    text(x = (posX + diffX/2), y = (posY + diffY) * 0.95, 
         labels = paste0("Modas (cm)"), cex = 0.8, font = 2)
  }
  else {
    text(x = tmpout2$lon, y = tmpout2$lat, labels = tmpout2$midlen, 
         col = 1, cex = CexPoint)
...
#     ubicacionMarcas <- which(marcas %in% temp_biom$LONGITUD_ESPECIE)
#     freq[ubicacionMarcas] <- temp_biom$FREC_SIMPLE
#     getModa <- modas(x = matrix(freq, nrow = 1), Lmin = 1, Lmax = 20,dL = 0.5,umbral = umbral)
#     
